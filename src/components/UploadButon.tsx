...
const UploadDropzone = ({isSubscribed}:{isSubscribed:boolean}) => {
  ...
  const startSimulatedProgress = () => {
    ...
    <Dropzone multiple={false} onDrop={async (acceptedFile)=>{
        ...
        console.log(res)
        if(!res){
            return toast.error("res went wrong", {
        ...
        if(!key){
            return toast.error("key went wrong", {
                description: "Please try again later"
            })
        }
            ...
            ({getRootProps, getInputProps, acceptedFiles}) => (
                <div {...getRootProps()} className='border h-64 m-4 border-dashed border-gray-300 rounded-lg'>
                    <div className='flex items-center justify-center h-full w-full'>
                        ...
                        <input {...getInputProps()} type="file" id='dropone-file' className='hidden' />
                        </label>
                    </div>
                </div>
            )
...
const UploadButon = ({isSubscribed}:{isSubscribed:boolean}) => {
